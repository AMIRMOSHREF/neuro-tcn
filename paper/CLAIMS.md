...
| **P1b** temporal / non-linear gain | DelayCAST on the selected K ≥ linear decoder on the selected K, where the linear decoder is the **time-resolved** one (`logreg_selected_units_windows`: mean, late-delay and four window rates per unit, the features of the model's wide path, which is warm-started at that very fit on the training trials); the mean-rate decoder `logreg_selected_units` and the within-pipeline ablations `criteria_linonly` (wide path alone) and `criteria_noskip` (deep path alone) are reported next to it | `criteria` run vs `logreg_selected_units_windows` (fallback `logreg_selected_units`) | paired Δ balanced accuracy | CI of Δ includes 0 |
...
| **P7** spectro-temporal information | full model (learned causal filterbank on the gated population rate, Gabor-initialised) > matched population-mean control (`popmean`: the same window, no spectral content); `nospec` and the fixed-bank variant reported when run | `criteria` vs `criteria_popmean` (and `criteria_nospec`) | paired Δ Left/Right balanced accuracy | CI includes 0 |
